...
export async function GET(request: Request) {
  ...
  const enhancedRequests = await Promise.all(
    requests.map(async (request) => {
      ...
      const requestedByUser = await client
        .db(DATABASE_NAME)
        .collection("users")
        .findOne(
          { walletAddress: request.requestedBy },
      ...
      const property = await client
        ...
        .findOne(
          ...
          { projection: { name: 1, location: 1, type: 1, price: 1, images: 1, title: 1 } }
        );
